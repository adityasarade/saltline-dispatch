...
| Asset | Origin | License / use basis | Notes |
| --- | --- | --- | --- |
| `docs/screenshots/*.webp` | Captured from the local production build on 17 September 2026 with Playwright, then encoded with `cwebp` 1.6.0 (quality 86 method 6 for the original eight, quality 80-82 for the three CITY HEAT stills) | Team-created product documentation | Eleven judge-facing states at 1280 × 720, 1280 × 1688 and 390 × 844: landing, five field calls, explicit Angle Lock, the editor carrying a real draw mark, the editor with the live sample reading ON THE LEAD, the night press run, publish, closing frame, the archive, the desk standing sheet, and the issue wall under city heat with its pinned notice and contradiction stamps. Every call preview is fully decoded in the capture. |
